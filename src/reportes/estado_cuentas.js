...
const { reporteCuentas } = require('../controller/deudas');
...
async function filtrar(req, res) {
  ...
  const deudas = await reporteCuentas(req.body)

  const tabla = deudas.map(deuda => {
    return {
      paciente: `${deuda.factura.paciente.nombres}, ${deuda.factura.paciente.apellidos}`,
      comprobante: deuda.factura.comprobante,
      fecha: deuda.factura.fecha,
      deuda: deuda.debe - deuda.haber
    }
  })
